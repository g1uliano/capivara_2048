...
class DailyRewardsNotifier extends Notifier<DailyRewardsState> {
  ...
  Future<void> claim(DateTime now) async {
    ...
    if (reward.lives > 0) await ref.read(livesProvider.notifier).addEarned(reward.lives);
    if (reward.undo1 > 0) await ref.read(inventoryProvider.notifier).add(ItemType.undo1, reward.undo1);
    if (reward.bomb2 > 0) await ref.read(inventoryProvider.notifier).add(ItemType.bomb2, reward.bomb2);
  ...
  Future<void> claimDouble(DailyReward original) async {
    if (original.lives > 0) await ref.read(livesProvider.notifier).addEarned(original.lives);
    if (original.undo1 > 0) await ref.read(inventoryProvider.notifier).add(ItemType.undo1, original.undo1);
    if (original.bomb2 > 0) await ref.read(inventoryProvider.notifier).add(ItemType.bomb2, original.bomb2);
  }
...
final dailyRewardsProvider = NotifierProvider<DailyRewardsNotifier, DailyRewardsState>(
