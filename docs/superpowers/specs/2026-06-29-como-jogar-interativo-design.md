# Como jogar — tela interativa 4×4 que ensina as regras de verdade

**Data:** 2026-06-29
**Fase:** 6 (polimento, l10n, acessibilidade, lançamento)
**Status:** aprovado, pronto para plano de implementação

> Nomenclatura: a funcionalidade é chamada **"Como jogar"** em todo texto visível
> ao usuário — nunca "Tutorial". Os nomes de arquivo e classe em código
> (`TutorialScreen`, `tutorial_*.dart`) seguem a convenção de nomes em inglês no
> código e ficam como estão; só a UI e a prosa usam "Como jogar".
...
1. ao deslizar o dedo, **todas** as peças do tabuleiro se movem juntas naquela
   direção;
2. só peças de **valor igual** se unem num mesmo bicho para evoluir.

A tela atual de "Como jogar" (`TutorialScreen`, 5 páginas) tem duas páginas
"interativas" (`tutorial_movement_page.dart` e `tutorial_fusion_page.dart`) que
usam o widget `tutorial_mini_board.dart` — uma **tirinha 1D de 2 células numa
única linha**. Essa tirinha é a raiz do problema: com no máximo 2 peças numa
linha, ela não consegue demonstrar nem "tudo desliza junto" nem "só os iguais se
unem num só", porque nunca há um tabuleiro com várias peças diferentes ao mesmo
tempo.

Pedido complementar do usuário: a tela **também** deve demonstrar as ferramentas
existentes (bomba, desfazer, vidas) e o que dá pra fazer com elas — não só
listá-las em cards de texto.

## Objetivo

Substituir as páginas interativas fracas por um **tabuleiro 4×4 guiado**
(mesmo tamanho do jogo) onde a criança aprende as regras jogando de verdade, e
transformar a página de ferramentas numa **demonstração interativa**
(toca-e-vê). Critério de sucesso: uma criança que nunca jogou consegue, ao final
do "Como jogar", mover o tabuleiro, unir dois iguais num mesmo bicho e entender
as três ferramentas — sem depender de um adulto explicando.
...
- `move(GameState, Direction)` — desliza tudo, une só os iguais, nasce peça
  nova, empilha estado anterior em `undoStack`;
- `removeTiles(GameState, positions)` — efeito da bomba;
- `undoStack` no `GameState` — base do desfazer.

**Cada página interativa cria sua própria instância local de `GameEngine` +
`GameState`** (com `Random` semeado para nascimentos previsíveis) e dirige o
motor de verdade, **sem tocar no `gameProvider` global**. Assim a tela nunca
diverge das regras reais e não duplica lógica.
...
- `BoardWidget` (`lib/presentation/widgets/board_widget.dart`) hoje lê
  ...
  efeito no jogo. A tela passa seu board local e fica pixel-idêntico ao jogo.
...
2. Tabuleiro guiado 4×4 (NOVO — substitui Mover + Juntar)
...
páginas 1 e 2, 0-indexadas: tabuleiro guiado e ferramentas).

### Página 2 — Tabuleiro guiado 4×4
...
- Board semeado com ~4 bichos, **todos de níveis diferentes** (impossível unir
  qualquer par), espalhados de modo que um swipe os faça deslizar visivelmente.
...
- Board semeado com **duas tanajuras na mesma linha/coluna** + ao menos um bicho
  **diferente** ao lado (para a criança ver que ele **não** se une).
- Dica: *"Deslize pra juntar as duas tanajuras num só bicho."*
- **Conclui** quando dois iguais se unem (detectado por aumento de `maxLevel` ou
...
- **Gradua** após ~2 uniões (ou ao alcançar nível 3) → mensagem *"Você pegou o
...
Em vez disso, a tela mantém `GameState`/`GameEngine` locais e reusa só os
widgets visuais:
...
- `BombGridOverlay` e `BombDimOverlay` hoje leem `gameProvider`/`notifier`.
  ...
  padrão do `BoardWidget`. A tela passa o estado local e usa **os mesmos
...
**Bomba**
- Board montado meio enroscado. Botão de bomba da própria tela com a dica
...
**Desfazer**
- A tela faz/mostra uma jogada e então: *"Errou? Toque em desfazer ↩"* →
...
> O **disparo** da bomba/desfazer é um botão simples da própria tela, não a
> `InventoryBar` completa (acoplada a `inventoryProvider`/contagens). Tudo que a
...
recebem `totalPages` por parâmetro). Se houver algum rótulo "Tutorial" visível ao
usuário (ex.: botão que abre a tela), trocar por "Como jogar" — verificar na
implementação.

## Testes

- **Widget test** novo: simula uma swipe no `tutorial_board` e confirma que o
  passo avança (gating do tabuleiro guiado) — o menor teste que falha se o
  gating quebrar.
- `GameEngine` já tem cobertura unitária (regras de mover/unir/remover) — não
  precisa de teste novo de regras.
- **Goldens:** os goldens da tela (Fase 3.5, `alchemist`) precisarão ser
...
- **Strings PT-BR hardcoded** nas páginas, seguindo o padrão atual (as páginas
  existentes não usam `intl`). Migrar para `intl` fica fora de escopo aqui; se
  desejado, é um passo separado que cobre a tela inteira.
- **Disparo das ferramentas é um botão simples da própria tela**, não a
  `InventoryBar` completa (acoplada a `inventoryProvider`). Os efeitos visíveis
  (dim, grade de seleção, explosão, rewind VHS) usam os widgets reais.
  (`// ponytail:` onde o botão de disparo vive.)
- A tela dirige `GameEngine`/`GameState` locais — **não** integra com vidas,
...
- Tudo em português simples, sem termos em inglês na UI ("deslize", "junte",
  "unir", "desfazer", "peça/bicho" — nunca "swipe", "merge", "tile", "undo").
- A funcionalidade se chama **"Como jogar"** na UI — nunca "Tutorial". Verbo das
  regras: **unir/juntar num mesmo bicho** — nunca "fundir".
