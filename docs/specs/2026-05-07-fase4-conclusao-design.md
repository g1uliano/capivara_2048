...
Dentro de `_LegendsRankingTab` (widget da aba "Lendas"), verifica `authControllerProvider`:

- **Logado →** conteúdo normal da aba
- **Não logado →** widget `_LoginRequiredBanner` centralizado:

```
     🔒
  Para ver o Ranking de Lendas
  você precisa estar conectado.

     [ Fazer login ]
```

O `_LoginRequiredBanner` é privado da `RankingScreen`. "Fazer login" → push `OnboardingAuthScreen(showSkip: false)`. Após pop com login efetuado, o `ConsumerWidget` da tab reconstrói automaticamente.
...
| `presentation/screens/ranking_screen.dart` | Existente | +`_LoginRequiredBanner` na aba Lendas |
